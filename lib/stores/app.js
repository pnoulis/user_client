...
const usePublicReducer = createPersistedReducer("app");
const
PUBLIC_SCHEMA = {
  cart: {meta: {total: 0}},
  loggedIn: false,
...
useApp =  () => {
  ...
  const [publicState, publicDispatch] = usePublicReducer(PUBLIC_REDUCER, PUBLIC_SCHEMA);
  const [privateState, privateDispatch] = useReducer(PRIVATE_REDUCER, PRIVATE_SCHEMA);
  const proxy = (action, ...payload) => {
    ...
    case "both":
      ...
      return setTimeout(() => publicDispatch(action), 100);
    default:
